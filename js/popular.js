...
function popularContainerFunc(){
    let url = 'https://gogoanime.herokuapp.com/popular';
    fetch(url)
        ...
        .then(result => {
            ...
            for (let x = 0; x < result.length / 2 - 1; x++) {
                ...
                bufferTwo += `<div class="col">
                    ...
                    ;
                popularContainer.innerHTML = `<h1>POPULAR ANIME</h1>
        ...
        .catch(err => {
            ...
            popularContainer.appendChild(notFound);
        });
    ...
    searching.addEventListener("submit", function (event) {
        ...
        fetch('https://raznime.herokuapp.com/search?keyw=' + buff + '')
            ...
            .then(result => {
                ...
                popularContainer.innerHTML = `<h1>search result : ${buff}</h1><div id="col-wrapper">${bufferThree}
               </div>`;
                document.querySelectorAll(".box-url>button").forEach((button) => {
                    button.onclick = function () {
                        animeDetailFunc(button.dataset.url);
...
function close() {
    menuBars.addEventListener('click', function () {
        ...
        <li title="POPULAR" class="active"> <a href="popular.html"><i class="fa-solid fa-star"></i></a> </li>
    </ul>`;
